...
int run_log_search(char *word){
    ...
    for(int i=last_commit_ID; i>0; i--){
        ...
        fclose(file);
        char *result=strstr(message,word);
        if(result!=0){
        printf("commit %s:\n",str);
        file=fopen(str,"r");
        while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if(i==1 && f==1)  printf("this word is not existed\n");
    }
    return 0;
}
...
int run_checkout(int commit_id){
    ...
    char commit_name[20];
    sprintf(commit_name, "%d", commit_id);
    char commit_folder[20];
    strcpy(commit_folder,commit_name);
    strcat(commit_folder,"f");

    FILE *file2;
    file=fopen(commit_name,"r");
    char line[MAX_LINE_LENGTH];
    int i=0;
    while (fgets(line, sizeof(line), file) != NULL) {
        i++;
